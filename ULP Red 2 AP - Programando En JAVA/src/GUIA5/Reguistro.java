...
public class Reguistro {
    ...
    public static void main(String[] args) {
        ...
        String[] nombres = { "Juan", "María", "Pedro", "Ana", "Luis", "Sofía", "Carlos", "Laura", "José", "Isabel",
                "Miguel", "Carmen" };
        String[] apellidos = { "Gómez", "Rodríguez", "López", "Fernández", "Pérez", "González", "Martínez", "Sánchez",
                "Romero", "Torres", "Vargas", "Ramírez" };

        Reguistro registro = new Reguistro();
        for (int j = 0; j < 12; j++) {
            for (int i = 0; i < 3; i++) {
                double milimetro = (0 + (Math.random() * ((1147 - 0) + 1)));
                registro.agregarRegistro(new Persona(nombres[(int) (Math.random() * 12)],
                        apellidos[(int) (Math.random() * 12)], milimetro), j, i);
            }
        }
